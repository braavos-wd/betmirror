...
We use a **Three-Tier Key System** to ensure your funds are never exposed to the server.

1.  **Owner Key (Yours):** Held in your MetaMask. Has root admin access. **Never stored by us.**
2.  **Session Key (Bot):** A restricted key generated in your browser and sent to the server. It can sign trades but cannot withdraw funds.
3.  **L2 API Key (CLOB):** Auto-generated by the bot using the Session Key. Used solely to authenticate with Polymarket's matching engine (HTTP/WS).
...
- [x] L2 CLOB Credential Auto-Generation
